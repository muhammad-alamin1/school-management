import React, { useState } from "react";
import axiosInstance from "../../../Hooks/axios";
import DashboardPanel from "../../Admin/DashboardPanel/DashboardPanel";

export default function AddExamSchedule() {
  const [error, setError] = useState();
  const [success, setSuccess] = useState();
  const [loading, setLoading] = useState(false);

  const [title, setTitle] = useState("");
  const [file, setFile] = useState(null);

  // file change
  const handleFileSelect = (event) => {
    setFile(event.target.files[0]);
  };

  // handle submit
  const handleSubmit = (e) => {
    e.preventDefault();

    // data
    const formData = new FormData();

    formData.append("title", title);
    if (file) {
      formData.append("file", file);
    }

    // axios post req
    axiosInstance
      .post(`/exam-schedule/add-exam/`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
          "Content-Type": "application/json",
        },
      })
      .then(function (response) {
        if (response.status === 200 || response.status === 201) {
          setSuccess(response.data.message);
          setError("");
        } else {
          setSuccess("");
          setError("Exam Schedule added failed.!");
        }
        setLoading(false);
      })
      .catch(function (error) {
        setSuccess("");
        setError("Exam Schedule added failed.!");
        setLoading(false);
      });
  };
  return (
    <>
      <DashboardPanel />
      <div className="sidebar-margin">
        <h3 className="my-4">Add Exam Schedule</h3>

        <form onSubmit={handleSubmit} encType="multipart/form-data">
          {success && (
            <div class="alert alert-success alert-dismissible fade show">
              <strong>Success!</strong> {success}
              <button
                type="button"
                class="btn-close"
                data-bs-dismiss="alert"
              ></button>
            </div>
          )}
          {error && (
            <div class="alert alert-danger alert-dismissible fade show">
              <strong>Error!</strong> {error}
              <button
                type="button"
                class="btn-close"
                data-bs-dismiss="alert"
              ></button>
            </div>
          )}
          {loading && (
            <div class="d-flex justify-content-center">
              <div class="spinner-border" role="status">
                <span class="sr-only">Loading...</span>
              </div>
            </div>
          )}
          <div className="row">
            <div className="col">
              <label for="fullName">Exam Title *</label>
              <input
                type="text"
                name="title"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="form-control"
                placeholder="Exam Title"
                required
              />
            </div>
            <div className="col">
              <label for="fullName">Chose file *</label>
              <input
                type="file"
                name="title"
                onChange={handleFileSelect}
                className="form-control"
                placeholder="Exam Title"
                required
                accept=".pdf,.doc"
              />
              <small className="text-muted">Allow Only .pdf & .doc file</small>
            </div>
          </div>
          <input
            type="submit"
            value="Submit"
            className="btn btn-outline-success my-4"
          />
        </form>
      </div>
    </>
  );
}
